...
`fmri_pipeline/registration_qc.py` provides a manifest-driven registration QC utility for aligned reference/coreg image pairs.
...
This implementation is intentionally duplicated from `fmri_utils` so that the
preprocessing workshop can run registration QC without installing a second
repository.
...
Rendering high-resolution APNGs for a full dataset should run as a compute job,
not on a login node. From the analysis directory:
...
Edit the allocation, email, and `project` path in `run_registration_qc.sh`,
then submit it:
...
The template builds the manifest and renders all available fMRIPrep subjects.
It also writes `manifest_one_row.csv`, which can be used for a quick one-run
smoke test when troubleshooting.

## Python API

```python
from registration_qc import RegistrationQcConfig, run_registration_qc
